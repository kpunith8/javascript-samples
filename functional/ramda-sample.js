...
// Ramda accepts source as second parameter, action to be performed as first paramter.
console.log(
  "map():",
  R.map((x) => x + 2, arr1)
);
...
console.log("both():", R.filter(R.both(isEven, isGreaterThan5), arr1));
...
const operatePipe = R.pipe(multiply, addOne, square);
...
const operateCompose = R.compose(square, addOne, multiply);
...
const titlesForYear = (books, year) => {
  // Would have used partial if the param to publishedInYear() are reversed
